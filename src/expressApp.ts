...
import { ExpNext, ExpRes, ExpReq } from '@customTypes/express'
...
export const createExpressApp = () => {
  ...
  /**
   * Make user's latest data available on every request
   */
  expressApp.use((req: ExpReq, _res: ExpRes, next: ExpNext) => {
    if (!req.session.user) {
      return next()
    }
    User.fetchByEmail(req.session.user.email)
      .then(user => {
        req.user = user
        next()
      })
